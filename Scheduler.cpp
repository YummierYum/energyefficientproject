...
unordered_map<MachineId_t, stateChange> curChangingState; // track machines currently undergoing s-state change
deque<TaskId_t> tasksWaiting; // tasks waiting to be assigned to a machine
...
double getMachineMaxMips(MachineWithVMs machine) {
...
double getMachineCurMips(MachineWithVMs machine, Time_t now) {
    double mips = 0.0;
...
// cur mips over max mips
double getMachineCurUtil(MachineWithVMs machine, Time_t now) {
    double curMips = getMachineCurMips(machine, now);
    double maxMips = getMachineMaxMips(machine);

    return curMips / maxMips;
}

// estimated utilization of task on machine
double getEstimatedTaskUtil(MachineWithVMs machine, TaskId_t task_id, Time_t curr) {
    ...
    return reqTaskMips / getMachineMaxMips(machine);
}
        ...
        for (int i = 0; i < 4; i++) { // cycle through every machine type
            if (machinesByCPUType[i].size() > 0) {
                ...
                for (MachineWithVMs* machine : running[i]) {
                    totalCompute += getMachineCurMips(*machine, now);
                    maxCompute += getMachineMaxMips(*machine);
                }
                ...
                if (totalRunningUtil < UTIL_THRESHOLD_IDLE) {
                    ...
                    while (runningIdx < running[i].size() && totalRunningUtil < UTIL_THRESHOLD_IDLE) {
                        ...
                        bool canBeIdled = getMachineCurMips(*machine, now) == 0.0;
                        if (canBeIdled && !curChangingState.count(machine->machine_id)) {
                            ...
                            double lostMips = getMachineMaxMips(*machine);
                    ...
                    while (j < end && numIntermediateMachines >(totalMachines * INIT_RUNNING)) {
                        ...
                        bool canBeOffed = getMachineCurMips(*machine, now) == 0.0;
